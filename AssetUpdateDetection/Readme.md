...
- Fingerprinting automatique du script Blazor
- Caching automatique des ressources statiques avec gestion de version

## Tests possibles

- Publier (toujours le serveur en Blazor), folder "MyBlazor10\MyBlazor10" :

`dotnet publish -c Release -o ./publish`

- Aller dans ./publish via cd ./publish, et lancer l'application :

`dotnet MyBlazor10.dll`

- Ouvrir un navigateur sur le port mentionné (http://localhost:5000 par défaut)

- Visualiser l'image initiale dans la page d'accueil (sans modification) ou ici : /img/batman-figurine.jpg
- Modifier l'image dans le dossier wwwroot/img/batman-figurine.jpg (par exemple en mettant une autre image)
- Republier l'application (dotnet publish -c Release -o ./publish)
- Normalement le fingerprint devrait changer grâce à :

```
<img src="@Assets["img/batman-figurine.jpg"]" alt="Batman" />
```

- L'image ne se rafraichit pas automatiquement sur une page déjà chargée mais sera rafraichie sur un refresh ou un rechargement de la page (authentification, etc...).
   
# Version avant .Net 10
